...
/** Register the command and data-route surfaces, each guarded on its own service. */
function enable(ctx) {
  registerCommand(ctx, ctx.get('settings'))
  registerRoutes(ctx)
}
    ...
    handler: async (invocation) => {
      ...
      try {
        if (sub === 'status') {
          ...
          const plan = planImagePayload(inventory, resolveImageBound(settings, provider))
